...
public class Controller {
    ...
    public void initialize(){
        ...
        memoryScheduler.add(new MemoryBlock(0,0,100));
        memoryScheduler.add(new MemoryBlock(0,600,850));
        memoryScheduler.add(new MemoryBlock(0,1250,1570));
        ...
        listMemory.addListener(new ListChangeListener<MemoryBlock>() {
            ...
            public void onChanged(Change<? extends MemoryBlock> c) {
                int memory = 0;
                for (MemoryBlock memoryBlock : memoryScheduler.getMemoryBlocks()) {
        ...
        exTask.Generate(processes);

        ExWork exWork = new ExWork();
        exWork.Work(processes);

        ExCheck exCheck = new ExCheck();
        exCheck.Check(processes);
    }
